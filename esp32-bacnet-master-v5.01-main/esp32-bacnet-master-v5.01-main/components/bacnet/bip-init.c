...
/* --- Add this to the bottom of bip-init.c --- */
                                 ...
                                 int32_t event_id, void *event_data)
{
    if (event_id == IP_EVENT_STA_GOT_IP) {
        ...
        /* Log the new IP for debugging */
        ESP_LOGI(TAG, "BACnet/IP Event: Got IP Address " IPSTR, IP2STR(&event->ip_info.ip));
        
        /* Update the BACnet stack with the new local IP */
        bip_set_addr(event->ip_info.ip.addr);
        
        /* Update the BACnet stack with the new broadcast address */
