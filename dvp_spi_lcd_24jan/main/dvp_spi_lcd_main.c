...
/* ================= Processing loop ================= */
    ...
    while (1) {
        ...
        // Check if a new frame has been captured by the callback
        if (g_frame_counter != last_processed_frame) {
            ...
            /* ---- 2. Heuristic gate (Reject bad frames) ---- */
            if (s.mean < 20 || s.mean > 230) {
                ESP_LOGI(TAG, "Frame %lu rejected (mean=%u)", last_processed_frame, s.mean);
                goto next;
            }
            ...
            /* ---- 4. Hex Dump for Python Verification ---- */
            // We do this EVERY 50 frames so you have time to copy it
            if ((last_processed_frame % 50) == 0) {
                printf("IMAGE_START\n");
                for (int i = 0; i < ML_W * ML_H; i++) {
                    printf("%02x", ml_y_buf[i]); 
                    // Add a newline every 96 pixels to keep the terminal readable
                    if ((i + 1) % ML_W == 0) printf("\n"); 
                }
                printf("IMAGE_END\n");
            }

            /* ---- 5. Normalize for ML (INT8 format) ---- */
            for (int i = 0; i < ML_W * ML_H; i++) {
                ml_input[i] = ((int8_t)ml_y_buf[i]) - 128;
            }

            /* ---- 6. Optional SPIFFS Save ---- */
            if ((last_processed_frame % SAVE_EVERY_N_FRAMES) == 0) {
                save_ml_frame_pgm(ml_y_buf, ML_W, ML_H, last_processed_frame);
            }

            ESP_LOGI(TAG, "Frame %lu processed (mean=%u)", last_processed_frame, s.mean);
        }
