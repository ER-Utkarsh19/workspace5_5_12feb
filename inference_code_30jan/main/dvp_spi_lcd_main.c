...
#define SAVE_EVERY_N_FRAMES 0  // 0 = Disable saving, just run AI
/* ================= Globals ================= */
...
void app_main(void)
{
    ...
    esp_cam_ctlr_dvp_pin_config_t pin_cfg = {
        ...
        .data_io = {
            EXAMPLE_DVP_CAM_D0_IO, EXAMPLE_DVP_CAM_D1_IO, EXAMPLE_DVP_CAM_D2_IO, EXAMPLE_DVP_CAM_D3_IO,
            EXAMPLE_DVP_CAM_D4_IO, EXAMPLE_DVP_CAM_D5_IO, EXAMPLE_DVP_CAM_D6_IO, EXAMPLE_DVP_CAM_D7_IO,
        },
    ...
    void *cam_buffer = esp_cam_ctlr_alloc_buffer(cam_handle, cam_buf_size, EXAMPLE_DVP_CAM_BUF_ALLOC_CAPS);
    if (!cam_buffer) {
    ...
    /* ---------- Grayscale buffers ---------- */
    g_y_buf = heap_caps_malloc(CAM_W * CAM_H, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!g_y_buf) {
        ESP_LOGE(TAG, "Y buffer alloc failed");
        return;
    }

    ml_y_buf = heap_caps_malloc(ML_W * ML_H, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!ml_y_buf) {
        ESP_LOGE(TAG, "ML Y buffer alloc failed");
        return;
    }

    ESP_LOGI(TAG, "Buffers allocated: Y=%d bytes, ML=%d bytes", CAM_W * CAM_H, ML_W * ML_H);
    ...
    ESP_ERROR_CHECK(esp_cam_ctlr_register_event_callbacks(cam_handle, &cbs, &cam_ctx));
    ESP_ERROR_CHECK(esp_cam_ctlr_enable(cam_handle));
    ESP_ERROR_CHECK(esp_cam_ctlr_start(cam_handle));

    ESP_LOGI(TAG, "Camera started (YUV422 to Y)");
    
    // IMPORTANT: Make sure model_setup() is called to initialize TFLM!
    model_setup(); 

    /* ================= OLD Data Collection Loop (COMMENTED OUT) ================= */
    /*
    while (1) {
        ...
        if (g_frame_counter != last_processed_frame) {
            ...
            // Note: Inference was optional here during collection
    ...
    */
    ...
    /* ================= NEW ACTIVE DETECTION LOOP ================= */
    /*
    while (1) {
        static uint32_t last_processed_frame = 0;

        // Only process if camera has delivered a new frame
        if (g_frame_counter != last_processed_frame) {
            last_processed_frame = g_frame_counter;

            // 1. Calculate Brightness Stats
            y_stats_t s;
            compute_y_stats(g_y_buf, CAM_W * CAM_H, &s);

            // 2. Resize Image for AI (96x96)
            resize_y_bilinear(g_y_buf, CAM_W, CAM_H, ml_y_buf, ML_W, ML_H);

            // 3. Smart Night Vision Boost
            uint8_t min_val = 255;
            uint8_t max_val = 0;

            // Scan image for min/max pixels
            for (int i = 0; i < ML_W * ML_H; i++) {
                if (ml_y_buf[i] < min_val) min_val = ml_y_buf[i];
                if (ml_y_buf[i] > max_val) max_val = ml_y_buf[i];
            }

            // Apply contrast stretch only if image is dark/low-contrast (Range < 60)
            // If range is large (daylight or bright light), skip boost to avoid noise.
            if ((max_val - min_val) > 5) { 
                float scale = 255.0f / (max_val - min_val);
                for (int i = 0; i < ML_W * ML_H; i++) {
                    float val = (ml_y_buf[i] - min_val) * scale;
                    ml_input[i] = (int8_t)(val - 128); // Convert to int8 (-128 to 127) for AI
                }
            } else {
                 // Too flat to boost, just center it
                 for (int i = 0; i < ML_W * ML_H; i++) {
                    ml_input[i] = (int8_t)(ml_y_buf[i] - 128);
                }
            }
            ...
            // 4. Run AI Prediction
            float prediction = model_run_inference(ml_input);
            
            // 5. Act on Results
            // Threshold 0.7 means 70% confidence it's "Dirty" (Resistor)
            if (prediction > 0.7) {
                 ESP_LOGE(TAG, "!!! DETECTED !!! Score: %.2f | Mean: %u", prediction, s.mean);
                 // Optional: Add GPIO code here to turn on a warning LED
            } else {
                 // Print a dot every 10 frames to show system is alive but found nothing
                 if (last_processed_frame % 10 == 0) {
                     printf("."); 
                     fflush(stdout);
                 }
            }
        ...
        // Fast delay to keep up with camera
        vTaskDelay(pdMS_TO_TICKS(10)); 
    }*/
    /* ================= NEW DEBUG LOOP (NO BOOST) ================= */
    while (1) {
        static uint32_t last_processed_frame = 0;
        ...
        if (g_frame_counter != last_processed_frame) {
            ...
            // 3. NO BOOST (Standard Normalization Only)
            // We strip out the fancy contrast stretching to stop the noise.
            for (int i = 0; i < ML_W * ML_H; i++) {
                // Convert 0..255 (uint8) to -128..127 (int8) directly
                ml_input[i] = (int8_t)(ml_y_buf[i] - 128);
            }
            ...
            // 5. Act on Results
            if (prediction > 0.8) { // Increased threshold slightly to 0.8
                 ESP_LOGE(TAG, "!!! DETECTED !!! Score: %.2f | Light Level: %u", prediction, s.mean);
            } else {
                 // Print the score even if it's low, so you can see if it hovers around 0.5 or 0.1
                 if (last_processed_frame % 10 == 0) {
                     printf("Clean (Score: %.2f) | Light: %u\n", prediction, s.mean);
                 }
            }
        }
